...
CLIENTSECRETS_LOCATION = "cred_dat.json"

with open(CLIENTSECRETS_LOCATION) as j:
  data = json.load(j)
...
class CredentialsReceivedError(Exception):
    pass
...
class RequestHandler(BaseHTTPRequestHandler):
  ...
  def do_GET(self):
    ...
    elif self.path.startswith('/oauth2callback?code='):
  ...
  def handle_redirected_url(self):
    ...
    raise CredentialsReceivedError()
...
def main():
  ...
  except CredentialsReceivedError as e:
    print("Got credentials needed.")
    server.socket.close()
